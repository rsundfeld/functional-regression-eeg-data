...
for(fold_iter in 1:5) {
  ...
  #summary(fdata2pc(x.v1.fdata, ncomp = 4))
  ...
  if(ajustar_modelos) {
    ...
    for(number.pcs.basis_x1 in 1:4){
    ...
    saveRDS(res.pc, paste('FingMov2 - FPCA - Fold', fold_iter,'.rds'))
  }  
  ...
  funcreg_model <- readRDS(paste('FingMov2 - FPCA - Fold', fold_iter,'.rds'))
...
metrics_df = as.data.frame(cbind(y_test_total, funcreg_total_predictions, funcreg_binary_predictions))
