...
setwd("C:\\Users\\Rodolfo\\Documents\\Rodolfo\\USP\\dissertacao\\datasets\\Finger Movements\\")
ajustar_modelos = F
...
for(fold_iter in 1:5) {
  ...
  x.v1.fdata <- fdata(x_train$V26) 
  x.v2.fdata <- fdata(x_train$V25)
  x.v3.fdata <- fdata(x_train$V27)
  x.v4.fdata <- fdata(x_train$V28)
  ...
  if(ajustar_modelos) {
    #2. optimize value of K (number of system basis) through GCV (don't use lambda=0 when deriv=1)
    optim.x.v1 <- optim.basis(fdataobj = x.v1.fdata, numbasis = seq(10, 50, by = 2),round(seq(5,time_points*0.7,length.out = 10)), 
                              lambda = c(0, 2^seq(-2, 4, length.out = 5)), type.basis = 'bspline', type.CV = GCV.S)

    optim.x.v2 <- optim.basis(fdataobj = x.v2.fdata, numbasis = seq(10, 50, by = 2),round(seq(5,time_points*0.7,length.out = 10)), 
                              lambda = c(0, 2^seq(-2, 4, length.out = 5)), type.basis = 'bspline', type.CV = GCV.S)

    optim.x.v3 <- optim.basis(fdataobj = x.v3.fdata, numbasis = seq(10, 50, by = 2),round(seq(5,time_points*0.7,length.out = 10)), 
                              lambda = c(0, 2^seq(-2, 4, length.out = 5)), type.basis = 'bspline', type.CV = GCV.S)

    optim.x.v4 <- optim.basis(fdataobj = x.v4.fdata, numbasis = seq(10, 50, by = 2),round(seq(5,time_points*0.7,length.out = 10)), 
                              lambda = c(0, 2^seq(-2, 4, length.out = 5)), type.basis = 'bspline', type.CV = GCV.S)
    ...
    for(number.beta.basis_x1 in round(seq(4, 12, by=2))){
      print(number.beta.basis_x1)
      for(number.beta.basis_x2 in round(seq(4, 12, by = 2))){
        for(number.beta.basis_x3 in round(seq(4, 12, by = 2))){
          for(number.beta.basis_x4 in round(seq(4, 12, by = 2))){
            ...
            for(i in 2:(2^n_features)){
              ...
              if (nrow == nrow_unique) {
                ...
                for(j in 1:k){
                  ...
                  basis.x1=create.fdata.basis(x.v1.fdata[-testIndexes,], type.basis="bspline", l=1:optim.x.v1.numbasis.opt)
                  basis.x2=create.fdata.basis(x.v2.fdata[-testIndexes,], type.basis="bspline", l=1:optim.x.v2.numbasis.opt)
                  basis.x3=create.fdata.basis(x.v3.fdata[-testIndexes,], type.basis="bspline", l=1:optim.x.v3.numbasis.opt)
                  basis.x4=create.fdata.basis(x.v4.fdata[-testIndexes,], type.basis="bspline", l=1:optim.x.v4.numbasis.opt)
                  
                  basis.b1=create.fdata.basis(x.v1.fdata[-testIndexes,], type.basis="bspline", l=1:number.beta.basis_x1)
                  basis.b2=create.fdata.basis(x.v2.fdata[-testIndexes,], type.basis="bspline", l=1:number.beta.basis_x2)
                  basis.b3=create.fdata.basis(x.v3.fdata[-testIndexes,], type.basis="bspline", l=1:number.beta.basis_x3)
                  basis.b4=create.fdata.basis(x.v4.fdata[-testIndexes,], type.basis="bspline", l=1:number.beta.basis_x4)
    ...
    best_row
    #write.csv2(beta_cv_matrix, "beta_cv_matrix_FingMov2_bspline.csv")
    ...
    basis.x1=create.fdata.basis(x.v1.fdata,type.basis="bspline", l=1:optim.x.v1.numbasis.opt);
    basis.b1=create.fdata.basis(x.v1.fdata,type.basis="bspline", l=1:as.numeric(best_row$b1)); 
    basis.x2=create.fdata.basis(x.v2.fdata,type.basis="bspline", l=1:optim.x.v2.numbasis.opt);
    basis.b2=create.fdata.basis(x.v2.fdata,type.basis="bspline", l=1:as.numeric(best_row$b2)); 
    basis.x3=create.fdata.basis(x.v3.fdata,type.basis="bspline", l=1:optim.x.v3.numbasis.opt);
    basis.b3=create.fdata.basis(x.v3.fdata,type.basis="bspline", l=1:as.numeric(best_row$b3)); 
    basis.x4=create.fdata.basis(x.v4.fdata,type.basis="bspline", l=1:optim.x.v4.numbasis.opt);
    basis.b4=create.fdata.basis(x.v4.fdata,type.basis="bspline", l=1:as.numeric(best_row$b4)); 
    basis.x=list("x1"=basis.x1, "x2"=basis.x2, "x3"=basis.x3, "x4"=basis.x4)                                     
    basis.b=list("x1"=basis.b1, "x2"=basis.b2, "x3"=basis.b3, "x4"=basis.b4)
    ...
    #summary(res.basis)
    saveRDS(res.basis, paste('FingMov2 - BSpline - FuncReg - Fold', fold_iter,'.rds'))
    
    #### 4.2. Lasso
    lasso_xtrain_matrix = as.matrix(cbind(x_train$V26, x_train$V25, x_train$V27, x_train$V28))
    ...
    saveRDS(model_lasso, paste('FingMov2 - BSpline - LASSO - Fold', fold_iter, '.rds'))
    
    #### 4.3. RF
    ml_df_train = as.data.frame(cbind(x_train$V26, x_train$V25, x_train$V27, x_train$V28, y_train))
    set.seed(42); model_rf <- randomForest(as.factor(y_train) ~ ., data=ml_df_train, 
                                        ntree = 50, nodesize = nrow(ml_df_train)*0.1)
    saveRDS(model_rf, paste('FingMov2 - BSpline - Random Forest - Fold', fold_iter,'.rds'))
  }
  
  #5. measure performance metrics
  
  ldata.test=list("df"=as.data.frame(y_test), 
                  "x1"=fdata(x_test$V26) - func.mean(fdata(x_train$V26)),
                  "x2"=fdata(x_test$V25) - func.mean(fdata(x_train$V25)),
                  "x3"=fdata(x_test$V27) - func.mean(fdata(x_train$V27)),
                  "x4"=fdata(x_test$V28) - func.mean(fdata(x_train$V28)))
  
  funcreg_model <- readRDS(paste('FingMov2 - BSpline - Funcreg - Fold', fold_iter,'.rds'))
  ...
  lasso_xtrain_colmeans = apply(lasso_xtrain_matrix,2,mean)
  lasso_xtrain_colsd = apply(lasso_xtrain_matrix,2,sd)
  lasso_model <- readRDS(paste('FingMov2 - BSpline - LASSO - Fold', fold_iter,'.rds'))
  lasso_xtest_matrix = as.matrix(cbind(x_test$V26, x_test$V25, x_test$V27, x_test$V28))
  ...
  rforest_model <- readRDS(paste('FingMov2 - BSpline - Random Forest - Fold', fold_iter,'.rds'))
  rforest_predictions <- predict(rforest_model,
                                 newdata=as.data.frame(cbind(x_test$V26, x_test$V25, x_test$V27, x_test$V28)), 
                                 type='prob')[,2]
...
###
#metrics <- rbind(funcreg_acc, funcreg_roc, lasso_acc, lasso_roc, rforest_acc, rforest_roc)
#metrics

#acc funcreg
#mean(metrics[1,]); sd(metrics[1,])
#roc auc funcreg
#mean(metrics[2,])#; sd(metrics[2,])
...
#acc rf
#mean(metrics[5,]); sd(metrics[5,])
#roc auc rf
#mean(metrics[6,])#; sd(metrics[6,])
