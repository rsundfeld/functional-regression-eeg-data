...
for(fold_iter in 1:5) {
  ...
  if(ajustar_modelos) {
    ...
    optim.x.v1 <- optim.basis(fdataobj = x.v1.fdata, numbasis = seq(3,49, by = 2),round(seq(5,time_points*0.7,length.out = 10)), 
                              lambda = c(0, 2^seq(-2, 4, length.out = 5)), type.basis = 'fourier', type.CV = GCV.S)

    optim.x.v2 <- optim.basis(fdataobj = x.v2.fdata, numbasis = seq(3,49, by = 2),round(seq(5,time_points*0.7,length.out = 10)), 
                              lambda = c(0, 2^seq(-2, 4, length.out = 5)), type.basis = 'fourier', type.CV = GCV.S)

    optim.x.v3 <- optim.basis(fdataobj = x.v3.fdata, numbasis = seq(3,49, by = 2),round(seq(5,time_points*0.7,length.out = 10)), 
                              lambda = c(0, 2^seq(-2, 4, length.out = 5)), type.basis = 'fourier', type.CV = GCV.S)
 
    optim.x.v4 <- optim.basis(fdataobj = x.v4.fdata, numbasis = seq(3,49, by = 2),round(seq(5,time_points*0.7,length.out = 10)), 
                              lambda = c(0, 2^seq(-2, 4, length.out = 5)), type.basis = 'fourier', type.CV = GCV.S)
            ...
            for(i in 2:(2^n_features)){
              ...
              if (nrow == nrow_unique) {
                ...
                for(j in 1:k){
                  ...
                  acc = append(acc, auc(roc(y_train[testIndexes], 
                                            predict(res.basis.cv, newx = ldata.valid.cv)))[[1]])#table.cv %>% diag() %>% sum () / table.cv %>% sum())
                }
    ...
    best_row
    #write.csv2(beta_cv_matrix, "beta_cv_matrix_FingMov2_fourier.csv")
    ...
    #summary(res.basis)
    saveRDS(res.basis, paste('FingMov2 - Fourier - FuncReg - Fold', fold_iter,'.rds'))
    ...
    saveRDS(model_lasso, paste('FingMov2 - Fourier - LASSO - Fold', fold_iter, '.rds'))
    ...
    saveRDS(model_rf, paste('FingMov2 - Fourier - Random Forest - Fold', fold_iter,'.rds'))
  }
  ...
  funcreg_model <- readRDS(paste('FingMov2 - Fourier - Funcreg - Fold', fold_iter,'.rds'))
  ...
  if(ajustar_modelos == F) {
    lasso_xtrain_matrix = as.matrix(cbind(x_train$V26, x_train$V25, x_train$V27, x_train$V28))
    lasso_xtrain_colmeans = apply(lasso_xtrain_matrix,2,mean)
    lasso_xtrain_colsd = apply(lasso_xtrain_matrix,2,sd)
  } 
  lasso_model <- readRDS(paste('FingMov2 - Fourier - LASSO - Fold', fold_iter,'.rds'))
  ...
  rforest_model <- readRDS(paste('FingMov2 - Fourier - Random Forest - Fold', fold_iter,'.rds'))
